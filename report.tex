...
\section{Navigation}
    ...
    \subsection{State Machine}
        ...
        \subsubsection{Junction and Truck Detection}
            ...
            \begin{figure}
                \begin{center}
                    \begin{subfigure}{0.5\linewidth}
                        \includegraphics[width=\linewidth]{charwidth.png}
                        \subcaption{Charachteristic pattern of widening (1), narrowing (2), and widening again (3) as we follows the inner line (turquoise) into the junction.}
                        \label{fig:charwidth}
                      \end{subfigure}%
                      \begin{subfigure}{0.5\linewidth}
                        \centering
    ...
    \subsection{Plate Reader Commands} \label{platereadercontrol}
        ...
        It was desired to be able to get better readings on the plates. This was accomplished by receiving a limited set of commands from the plate reader node. Whenever the plate reader would detect a plate, it would send a command to the controller to move into a StopTurn{Left/Right} state. 
